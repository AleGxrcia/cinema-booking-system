...
public sealed class ChangeMovieStatusCommandHandler : ICommandHandler<ChangeMovieStatusCommand>
{
    private readonly IMovieRepository _movieRepository;
    private readonly IUnitOfWork _unitOfWork;

    public ChangeMovieStatusCommandHandler(IMovieRepository movieRepository, IUnitOfWork unitOfWork)
    {
        _movieRepository = movieRepository;
        _unitOfWork = unitOfWork;
    }

    public async Task<Result> Handle(ChangeMovieStatusCommand request, CancellationToken cancellationToken)
    {
        ...
        var result = movie.ChangeStatus(request.NewStatus);
        if (result.IsFailure)
            return result.Error;
